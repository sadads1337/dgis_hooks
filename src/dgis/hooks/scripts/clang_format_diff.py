...
def entry_point():
    ...
    parser.add_argument(
        "-i", action="store_true", default=False, help="apply edits to files instead of displaying a diff"
    )
    ...
    parser.add_argument("-binary", default="clang-format", help="location of binary to use for clang-format")
    parser.add_argument("-filediff", help="path to file with diff")
    parser.add_argument("-filesrc", help="path to source file")
    parser.add_argument("-workdir", default=".", help="path to work-dir")
...
def clang_format(args):
        ...
        with open(os.path.join(args.filediff), "r") as filediff:
            for line in filediff.readlines():
                ...
                if match:
                    ...
                    lines_by_file.setdefault(args.filesrc, []).extend(["-lines", str(start_line) + ":" + str(end_line)])
    elif args.filesrc:
    ...
    for filename, lines in lines_by_file.items():
        if args.verbose:
            print(
                "Formatting",
                os.path.join(
                    os.path.relpath(os.path.dirname(filename), os.path.abspath(args.workdir)),
        ...
        command = [
            args.binary,
            os.path.join(
                os.path.relpath(os.path.dirname(filename), os.path.abspath(args.workdir)), os.path.basename(filename)
            ),
        ]
        if args.i:
            command.append("-i")
    ...
    diff = difflib.unified_diff(
        code, formatted_code, args.filesrc, args.filesrc, "--- (BAD CODE)", "+++ (GOOD CODE)", n=1
    )
...
def caller_clang_binary(command, work_dir):
    p = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=sys.stderr, stdin=subprocess.PIPE, cwd=work_dir)
